...
$TARGET_EMAIL = array("^enquiries@phwebs\.co\.uk$");

            /* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","enquiries@phwebs.co.uk");
...
$TEMPLATEURL = "http://www.phwebs.co.uk";                  // default; no template URL
...
class   JSON
{
    function    _Format($m_val)
    {
        ...
        {
            for ($ii = 0 ; $ii < count($m_val); $ii++)
                $m_val[$ii] = $this->_Format($m_val[$ii]);
            $s_value = "[".implode(",",$m_val)."]";
        }
        else
            $s_value = "null";
        return ($s_value);
...
function    IsAjax()
{
    global  $SPECIAL_VALUES,$aFormVars;
